...
import 'package:safemap/utils/Key.dart';
...
class _MapViewState extends State<MapView> {
 ...
 List<LatLng> polylineCoordinates = [];
        ...
        setState(() {
         // _currentPosition.longitude =";
          ...
          mapController.animateCamera(
            CameraUpdate.newCameraPosition(
              CameraPosition(
                target: LatLng(33.892166 , 9.561555),
                zoom: 5.0,
              ),
            ),
          );
  ...
  _createPolylines(Position start, Position destination) async {
    ...
    print(polylineCoordinates.toString());
    print("ffffff");
    //print(_circles.intersection(other));

/*    for (int i = 0; i < _markers2.length - 1; i++) {
      if (_circles
          .intersection(Set<Polyline>.of(polylines.values))
          .length!=0) {
...
test()
{double _dangerDistance = 0.0;
for (int i = 0; i < _markers2.length ; i++) {
 // print(_markers2.elementAt(i));
     for (int j = 0; j < polylineCoordinates.length ; j++) {
       _dangerDistance = _coordinateDistance(
         polylineCoordinates[j].latitude,
         polylineCoordinates[j].longitude,
         _markers2.elementAt(i).position.latitude,
         _markers2.elementAt(i).position.longitude,
       );

     print(_dangerDistance.toStringAsFixed(2));
     }
   }
}
    ...
    rpt.then((value) {
      setState(() {
        ...
        value.forEach((element) {
          ...
          _circles.add(Circle(
              circleId: CircleId(element["longitude"]),
